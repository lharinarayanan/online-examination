...
$todo_id= $_GET['id'];
$ses_empho = mysqli_query($conn,"SELECT name,email,phone FROM student where name='$todo_id'");
$row = mysqli_fetch_array($ses_empho,MYSQLI_ASSOC);
$name=$row['name'];
$email=$row['email'];
$phone=$row['phone'];
$myusername=$myemail=$mytelephone="";
if($_SERVER["REQUEST_METHOD"] == "POST") {
      // username and password sent from form 
      $myusername = mysqli_real_escape_string($conn,$_POST['uname']);
      $myemail = mysqli_real_escape_string($conn,$_POST['email']);
      $mytelephone = mysqli_real_escape_string($conn,$_POST['tel']);
      $sql = "UPDATE student SET name='$myusername',email='$myemail',phone='$mytelephone' WHERE name='$name'  ";
      $sq = "UPDATE result SET name='$myusername' WHERE name='$name' ";
      $_SESSION['login_user'] = $myusername;
       if (mysqli_query($conn, $sql)) {
      	header("location: adminstudentedit.php");
    } 
 else {
